...
class VendorAgent:
    """Audit agent wrapper for vendor-related investigations."""
        ...
        *,
        ...
        if db is not None:
            ...
            try:
                ...
                return {
                    "success": False,
                    "selected_agent": "vendor_agent",
                    "user_query": query,
                    "error": "Database query failed. Check AUDIT_DATABASE_URL and PostgreSQL connectivity.",
                    "details": str(exc),
                }

        result["selected_agent"] = (routing_info or {}).get("agent", "vendor_agent")
...
if __name__ == "__main__":
    user_query = input("Enter vendor query: ")
    response = VendorAgent().run(user_query)
