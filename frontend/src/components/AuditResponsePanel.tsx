...
function RiskScoreBar({ score, rating }: { score: number; rating: string }) {
  const color = rating === 'HIGH' ? 'var(--accent-red)' : rating === 'MEDIUM' ? 'var(--accent-amber)' : 'var(--accent-green)';
  return (
    <div>
...
export function AuditResponsePanel({ response, onCitationSelect }: Props) {
  ...
  const findingSummary = txt(
    response.finding.summary || response.investigation_summary || response.transaction_summary || response.vendor_summary || response.final_response,
    'No summary provided.'
  );
  ...
  const evidenceMetrics = useMemo(() => [
    ...
    { label: 'Transactions Reviewed', value: response.investigation_metrics?.transactions_reviewed ?? response.structured_evidence.length, color: 'var(--accent-amber)' },
  ], [response]);

  return (
    <div className="response-panel">
      {/* Header with tabs and risk */}
      <div className="response-panel-header">
        <div className="flex-between" style={{ flexWrap: 'wrap', gap: '0.75rem' }}>
          <div>
            <p className="eyebrow" style={{ marginBottom: '0.3rem' }}>Investigation Result</p>
            <h2 style={{ fontSize: '1.15rem', fontWeight: 700 }}>{findingTitle}</h2>
          </div>
          <div className="flex-row" style={{ gap: '0.6rem', flexShrink: 0 }}>
            <span className={`risk-pill risk-${(response.risk_rating || 'low').toLowerCase()}`}>
              {response.risk_rating || 'LOW'}
            </span>
            <span className="small-copy" style={{ color: 'var(--text-muted)' }}>Score: {response.risk_score}</span>
          </div>
        </div>
      ...
      <div className="response-panel-body">
          ...
          <div className="stack">
            ...
            <div className="grid-3" style={{ gap: '1rem' }}>
              ...
              <div className="card-sm">
                ...
                <div className={`risk-pill risk-${(response.risk_rating || 'low').toLowerCase()}`} style={{ marginBottom: '0.75rem', fontSize: '1rem', fontWeight: 800 }}>
                  {response.risk_rating || 'LOW'}
                </div>
            ...
            {/* Evidence Metrics */}
            <div>
              <p className="label" style={{ marginBottom: '0.75rem' }}>Evidence Metrics</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.75rem' }}>
                {evidenceMetrics.map(({ label, value, color }) => (
                  <div key={label} className="card-sm" style={{ textAlign: 'center', borderTop: `2px solid ${color}` }}>
                    ...
                    <div className="small-copy" style={{ marginTop: '0.25rem' }}>{label}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {tab === 'evidence' && (
          <div className="stack">
                ...
                <div className="stack-sm" style={{ marginTop: '0.5rem' }}>
                  {response.recommendations.map((rec, i) => (
                    <div key={i} className="flex-row" style={{ alignItems: 'flex-start', gap: '0.6rem' }}>
                      <span style={{ background: 'rgba(59,130,246,0.15)', color: 'var(--accent-blue)', borderRadius: '50%', width: 20, height: 20, display: 'grid', placeItems: 'center', fontSize: '0.72rem', fontWeight: 700, flexShrink: 0 }}>
                        {i + 1}
                      </span>
            ...
            {/* Citations */}
            <div>
              <p className="label" style={{ marginBottom: '0.6rem' }}>Citations ({response.citations.length})</p>
