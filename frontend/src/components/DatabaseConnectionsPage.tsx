...
export function DatabaseConnectionsPage() {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      setTestMessage(result.success ? `Connection successful. ${result.schemas.length} schemas and ${result.tables.length} tables detected.` : result.message);
    } catch (err) {
    ...
    <div className="stack" style={{ gap: '1.25rem' }}>
      <div className="flex-between" style={{ flexWrap: 'wrap', gap: '0.75rem' }}>
        <div>
          ...
          <h1 style={{ fontSize: '1.55rem', fontWeight: 800 }}>Database Connections</h1>
          <p className="body-copy" style={{ marginTop: '0.35rem' }}>
            Add a PostgreSQL source, test it, and choose which schema and tables the audit assistant should use.
          </p>
        </div>
        {activeConnection && (
          <div className="card-sm" style={{ minWidth: 260 }}>
            ...
            <p className="small-copy">{activeConnection.database_type.toUpperCase()} | {activeConnection.host}:{activeConnection.port}</p>
          </div>
        )}
      </div>
      ...
      {error && (
        <div className="card-sm" style={{ borderLeft: '3px solid var(--accent-red)' }}>
          <p className="label" style={{ color: 'var(--accent-red)' }}>Connection Error</p>
      ...
      <div className="grid-2" style={{ gap: '1rem', alignItems: 'start' }}>
        <div className="card">
          <p className="label" style={{ marginBottom: '0.8rem' }}>Add Connection</p>
          <div className="stack-sm">
            <input className="input" placeholder="Connection name" value={form.connection_name} onChange={(e) => updateField('connection_name', e.target.value)} />
        ...
        <div className="card">
          <p className="label" style={{ marginBottom: '0.8rem' }}>Saved Connections</p>
          {loading ? (
            <p className="body-copy">Loading connections...</p>
          ) : connections.length === 0 ? (
            <p className="body-copy">No database connections saved yet.</p>
          ) : (
            <div className="stack-sm">
              {connections.map((connection) => (
                <div key={connection.connection_id} className="card-sm" style={{ borderLeft: connection.is_default ? '3px solid var(--accent-blue)' : '3px solid transparent' }}>
                  <div className="flex-between" style={{ gap: '0.75rem', flexWrap: 'wrap' }}>
                    <div>
                      <strong>{connection.connection_name}</strong>
                      <p className="small-copy">{connection.host}:{connection.port} · {connection.database_name}</p>
        ...
        <div className="card">
          <div className="flex-between" style={{ marginBottom: '0.8rem', flexWrap: 'wrap', gap: '0.75rem' }}>
            <div>
              ...
              <p className="small-copy">{selectedConnection.database_type.toUpperCase()} | {selectedConnection.host}:{selectedConnection.port} | {selectedConnection.database_name}</p>
            </div>
            ...
            <div className="card-sm">
              ...
              <div className="stack-sm">
                {schemas.length > 0 ? schemas.map((schema) => (
                  <label key={schema.schema_name} className="flex-between" style={{ gap: '0.75rem', padding: '0.45rem 0', borderBottom: '1px solid var(--border)' }}>
            ...
            <div className="card-sm">
              ...
              <div className="stack-sm">
                {tables.length > 0 ? tables.map((table) => (
                  <label key={`${table.schema_name}.${table.table_name}`} className="flex-between" style={{ gap: '0.75rem', padding: '0.45rem 0', borderBottom: '1px solid var(--border)' }}>
                    <span style={{ display: 'flex', flexDirection: 'column' }}>
